...
path1 = './datasets/3d_img_process'
# path2 = "/data2/gayrat/vs-projects/3d-img-processing/CT_64"
path2 = "./datasets/3d_img_process/CT_64"

dirs = os.listdir( path1 )
dirs = [d + '/' + jpg for d in dirs for jpg in os.listdir(path1 + '/' + d)]
print("----------------------------------------------------------")
...
for item in tqdm(dirs):
    if os.path.isfile(path1+'/'+item):
        ...
        cv2.imwrite(os.path.join(path2, item.split('/')[1][:-4] + '.jpg'), resized_image) # item[:-4], str(count)
        # count +=1
        # break
cv2.waitKey(0)
print("")
